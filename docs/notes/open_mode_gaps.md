...
- （最新）前端 A 股板块热力已改为直接消费 `a_share_short_term`（强势/弱势/资金榜）构建热力图，全球页也增加独立“美股焦点”卡片，避免美股落入“另类资产”场景；数据源标签统一移至页眉，显示模式/更新时间/缺数提示。
...
- （最新）重新梳理分发：全球场景包含区域脉搏 + 指数动能 + 美股焦点；另类场景改为数字资产 + 大宗/避险 + 汇率/摘要。视觉已换用 Space Grotesk/玻璃态网格背景，但仍需按设计稿进一步拆场景与动效。
